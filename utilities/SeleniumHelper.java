...
import com.UAT.SAMUI.automation.steps.SAMSteps;
...
public class SeleniumHelper {
	 ...
	 * @author
	 */
	public static RemoteWebDriver createWebDriverSauce(String browser,cucumber.api.Scenario scenario) {
		DesiredCapabilities capabilities = null;
		String USERNAME = System.getenv("sauce_username");
		String ACCESS_KEY = System.getenv("sauce_accesskey");
		...
		case "chrome":
			...
			capabilities.setCapability(CapabilityType.VERSION, "66");
		...
		case "ie":
			...
			capabilities.setCapability("iedriverVersion", "2.53.0");
        ...
        String UserStoryName = scenario.getId().split(";")[0];
        String FeatureName = UserStoryName.split("-")[0].toUpperCase();
        String jobName = "SAM Test Execution of [" + FeatureName + ":" + scenarioName + "]" ;
        log.info(jobName);
	...
	public static boolean findElementforInvisible(WebDriver driver, String[] elementProperties, long timeOutSeconds, 
            ExplicitConditions conditions)
        {
            try{
             switch (ElementLocator.valueOf(elementProperties[0].toUpperCase()))
                {
                    case ID:
                        return waitTillElementInvisible(driver, By.id(elementProperties[1]), timeOutSeconds, conditions);
                    case CLASSNAME:
                        return waitTillElementInvisible(driver, By.className(elementProperties[1]), timeOutSeconds, conditions);
                    case TAGNAME:
                        return waitTillElementInvisible(driver, By.tagName(elementProperties[1]), timeOutSeconds, conditions);
                    case NAME:
                        return waitTillElementInvisible(driver, By.name(elementProperties[1]), timeOutSeconds, conditions);
                    case XPATH:
                        return waitTillElementInvisible(driver, By.xpath(elementProperties[1]), timeOutSeconds, conditions);
                    case CSSSELECTOR:
                        return waitTillElementInvisible(driver, By.cssSelector(elementProperties[1]), timeOutSeconds, conditions);
                    case PARTIALLINKTEXT:
                        return waitTillElementInvisible(driver, By.partialLinkText(elementProperties[1]), timeOutSeconds, conditions);
                    case LINKTEXT:
                        return waitTillElementInvisible(driver, By.linkText(elementProperties[1]), timeOutSeconds, conditions);
                }
            }
            catch(Exception e)
            {
            	log.info(AutomationCore.testName + ": Element with locator "+ ElementLocator.valueOf(elementProperties[0].toUpperCase()) + " is not " + conditions + " . Waited for " + timeOutSeconds + " seconds for the element");
            	Assert.fail("Element with locator "+ ElementLocator.valueOf(elementProperties[0].toUpperCase()) + " is not " + conditions + " . Waited for " + timeOutSeconds + " seconds for the element");            	        
            }
			return false;
	...
	public static  boolean waitTillElementInvisible(WebDriver driver, By by, long timeOutInSeconds, ExplicitConditions condition)
    {
        try
        {
        	//threadSleep(5000);
            ...
            switch (condition)
            {
            	...
            	case INVISIBLE:
	            	boolean ele = wait.until(ExpectedConditions.invisibilityOfElementLocated(by));
	            	log.info(AutomationCore.testName + ": Element with locator "+ by + " is invisible");
	                return true; 
            }
        }
        catch (Exception e)
        {
        	log.info(AutomationCore.testName + ": Element with locator "+ by + " is not " + condition + " . Waited Rao" + timeOutInSeconds + " for the element");
        	return false;
        }
		return false;
	...
	public static void quitDriver() {
		com.UAT.SAMUI.automation.pages.BasePage.logInState = false;
	...
	public static void RadioButtonSelection(List<WebElement> RadBtn, String AttributeVal, String IgnoreCaseVal) {
		...
		for (int i = 0; i < RadRPC.size(); i++) {
			...
			if (value.equalsIgnoreCase(IgnoreCaseVal)) {
				System.out.println(value+"========"+IgnoreCaseVal);
				local_radio.click();
				break;
			}
		}
